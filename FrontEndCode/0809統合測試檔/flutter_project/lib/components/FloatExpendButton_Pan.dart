...
class FloatExpendButtonPan extends StatefulWidget 
{
  ...
  (
    this.iconList,
    {
      ...
      this.MainAnimatedIcon = AnimatedIcons.view_list,
      this.iconsize = 15,
      this.type = ButtonType2.Left
    }
  );
  @override
  // ignore: library_private_types_in_public_api
  _FloatExpendButtonPanState createState() => _FloatExpendButtonPanState();
}

//旋轉變換按鈕 向上彈出的效果
class _FloatExpendButtonPanState extends State<FloatExpendButtonPan>with SingleTickerProviderStateMixin 
{
  ...
  {
    if (widget.type == ButtonType2.Right || widget.type == ButtonType2.Bottom) 
    {
  ...
  {
    if (widget.type == ButtonType2.Left || widget.type == ButtonType2.Right) 
    {
  ...
  {
    if (widget.type == ButtonType2.Top || widget.type == ButtonType2.Bottom) 
    {
      return _fabtween.value * (i + 1);
    } 
    else 
  ...
  {
    if (widget.type == ButtonType2.Top) 
    {
      return Alignment.bottomCenter;
    } 
    else if (widget.type == ButtonType2.Left) 
    {
      return Alignment.centerRight;
    } 
    else if (widget.type == ButtonType2.Bottom) 
    {
  ...
  Widget build(BuildContext context) 
  {
    ...
    (
      ...
      children: 
      [
        widget.type == ButtonType2.Left || widget.type == ButtonType2.Right
            ? SizedBox
            (
...
enum ButtonType2 { Left, Right, Top, Bottom }